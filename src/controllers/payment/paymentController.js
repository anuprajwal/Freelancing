...
const { payments, appointments, doctorProfile, transfer: TransferModel } = require("../../../models");
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (!doctor || doctor.kyc_status !== "verified")
      return res.status(400).json({ message: "Doctor not eligible for payments" });
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Verification received" });
  } catch (err) {
    ...
    switch (event.event) {
      ...
      case "transfer.processed": {
        ...
        await TransferModel.update(
          { status: "processed", razorpay_transfer_id: t.id },
          { where: { order_id: t.order_id } }
        );

        await payments.update(
          { payment_status: "transfer_completed" },
          { where: { transaction_id: t.order_id } }
        );
