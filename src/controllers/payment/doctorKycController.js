...
exports.uploadKycDocument = async (req, res) => {
  try {
    ...
    // upload to Razorpay Files
    const docId = await uploadDocument(tmpPath, type);

    // Store file id
    if (type === "pan") doctor.pan_doc_id = docId;
    if (type === "bank_account") doctor.bank_doc_id = docId;
    if (type === "address") doctor.address_doc_id = docId;

    // Save any bank details if provided
    ...
    // If all docs + details present, call updateKyc (this will attach KYC to account)
    if (doctor.pan_doc_id && doctor.bank_doc_id && doctor.address_doc_id && doctor.pan_number && doctor.account_number && doctor.ifsc_code && doctor.address_line1) {
