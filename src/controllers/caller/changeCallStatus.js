...
const changeCallStatus = async (req, res) => {
  try {
    ...
    if (!VALID_STATUSES.includes(call_status)) {
      return res.status(400).json({ error: "Invalid call_status value" });
    }
    ...
    const { call_initiated_by, call_made_to, call_status: currentStatus } = callData;
    ...
    const TERMINAL_STATES = ["Call Rejected", "Call Complete"];

    if (TERMINAL_STATES.includes(currentStatus)) {
      return res.status(400).json({ error: "Call already completed or rejected" });
    }

    if (currentStatus === "Call Initialised" && call_status === "Call on hold") {
      return res.status(400).json({ error: "Cannot put an un-answered call on hold" });
    }
