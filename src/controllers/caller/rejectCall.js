...
const rejectCall = async (req, res) => {
  try {
    ...
    if (callData.call_status !== "Call Initialised") {
      return res.status(400).json({ error: "Call cannot be rejected now" });
    }
    ...
    await callHistoryDoc.set(
      {
        ...
        call_rejected_by: id,
        call_rejected_at: new Date().toISOString()
      },
      { merge: true }
    );

    return res.status(200).json({ message: "Call rejected successfully" });
