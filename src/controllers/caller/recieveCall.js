...
const receiveCall = async (req, res) => {
  try {
    ...
    // ---------------- Validation ----------------
    if (!call_id || !answer) {
      return res.status(400).json({ error: "call_id and answer are required" });
    }
    ...
    if (callData.call_status !== "Call Initialised") {
      return res.status(400).json({ error: "Call is not in initialised state" });
    }

    // ---------------- Ensure This User Is The Receiver ----------------
    if (callData.call_made_to !== id) {
      return res.status(403).json({ error: "You are not the receiver of this call" });
    }
    ...
    return res.status(200).json({ message: "Call accepted successfully" });
