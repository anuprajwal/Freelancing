const { doctorProfile, organisationProfile, User } = require("../../../models");


const approveDoctors = async (req, res)=>{
    const {id, scope} = req.admin
    ...
    if (!doctor_id){
        return res.status(404).json({error:"doctor_id not found in the body"})
    }
...
const approveHospitals = async (req, res)=>{
    ...
    if (!org_id){
        return res.status(404).json({error:"org_id not found in the body"})
    }
